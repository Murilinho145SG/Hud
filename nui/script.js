...
window.addEventListener('message', function (event) {
    ...
    switch (response.action) {
        ...
        case 'update':
            ...
            $('.hud .info .item#life .status').css(
                'height',
                `${response.life}%`,
            )
            if (response.shield > 0) {
                $('.hud .info .item#shield .status').css(
                    'height',
                    `${response.shield}%`,
                )
                $('.hud .info .item#shield').css('display', 'flex')
            } else $('.hud .info .item#shield').css('display', 'none')
            $('.hud .info .item#hunger .status').css(
                'height',
                `${response.hunger}%`,
            )
            $('.hud .info .item#thirst .status').css(
                'height',
                `${response.thirst}%`,
            )
